...
For this research, we downloaded the data and cleaned and analyzed it by using Rstudio. To run each file automatically, we created a makefile. The instructions are available on [http://tilburgsciencehub.com/](https://tilburgsciencehub.com):
...
```
4. Set your working directory to the just cloned folder using:
```
cd How-does-Airbnb-pricing-differ-between-stay-duration
```
6. Then in your command line type:
```
make
```
...
### Interpretation
To investigate the effect of stay duration on Airbnb prices across Amsterdam, Tokyo and London, considering review scores and room type, The linear regression results per city can be found below: 
#### Amsterdam
...
For London listings the p-value is 0.0000124 < 0.05. This indicates that there is a significant difference in price between the listing with a short-stay vs long-stay in London. The main effect stay_type_dummy on the price suggests that on average a ‘long-stay’ is 56.8 Pounds lower than ‘short-stay’. In contrast to listings located in Amsterdam and Tokyo, hotel and shared room options do long term stay opportunities. 
